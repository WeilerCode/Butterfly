    ...
    <div class="row">
        <div class="col-md-4">
            <div class="box box-success">
                ...
                <div class="box-body">
                    <form role="form" action="{{ route('admin-manage-permissions-edit-group-post', ['id' => $group->id]) }}" method="post">
                        {{ csrf_field() }}
                        <div class="form-group @if($error = getValidationErrorForTemplate($errors, 'name')){{ 'has-error' }}@endif">
                            ...
                            <input type="text" class="form-control" name="name" value="{{ old('name') ? old('name') : $group->name }}" placeholder="分组名称-可填写本地化名称">
                            {!! $error !!}
                        </div>
                        <div class="form-group @if($error = getValidationErrorForTemplate($errors, 'lv')){{ 'has-error' }}@endif">
                            <label>级别 <span class="asterisk">*</span></label>
                            <input type="text" class="form-control" name="lv" value="{{ old('lv') ? old('lv') : $group->lv }}" placeholder="权限级别,数字越小权限越大,必修为整数.">
                            {!! $error !!}
                        </div>
                        <div class="form-group myColor @if($error = getValidationErrorForTemplate($errors, 'color')){{ 'has-error' }}@endif">
                            ...
                            <div class="input-group myColor col-xs-6">
                                <input type="text" name="color" value="{{ old('color') ? old('color') : $group->color }}" class="form-control"/>
