                ...
                <div class="box-body">
                    <form role="form" action="{{ route('admin-manage-menu-edit-post', ['id' => $thisMenu->id]) }}" method="post">
                        ...
                        <div class="form-group @if($error = getValidationErrorForTemplate($errors, 'name')){{ 'has-error' }}@endif">
                            ...
                            <input type="text" class="form-control" name="name" value="{{ old('name') ? old('name') : $thisMenu->name }}" placeholder="{{ getLang('fieldsMenu.columnNameHelp') }}">
                            {!! $error !!}
                        </div>
                        <div class="form-group @if($error = getValidationErrorForTemplate($errors, 'routeName')){{ 'has-error' }}@endif">
                            ...
                            <input type="text" class="form-control" name="routeName" value="{{ old('routeName') ? old('routeName') : $thisMenu->routeName }}" placeholder="{{ getLang('fieldsMenu.routeNameHelp') }}">
                            {!! $error !!}
                        </div>
                        <div class="form-group @if($error = getValidationErrorForTemplate($errors, 'display')){{ 'has-error' }}@endif">
                            ...
                            <input type="checkbox" name="display" @if($thisMenu->display) checked @else checked-false @endif data-size="small" data-on-text="{{ getLang('Tips.showON') }}" data-off-text="{{ getLang('Tips.showOFF') }}" data-label-width="0" data-on-color="success" data-off-color="danger">
                            {!! $error !!}
                        </div>
                        <div class="form-group @if($error = getValidationErrorForTemplate($errors, 'listOrder')){{ 'has-error' }}@endif">
                            ...
                            <input name="listOrder" value="{{ old('listOrder') ? old('listOrder') : $thisMenu->listOrder }}" class="form-control" type="text" placeholder="{{ getLang('fieldsMenu.sortHelp') }}">
                            {!! $error !!}
                        </div>
